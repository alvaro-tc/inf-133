def test_register_user(test_client):
    new_user = {"username": "testadmin", "password": "testpassword", "roles": '["admin"]'}
    response = test_client.post("/api/register", json=new_user)
    assert response.status_code == 201


def test_register_duplicate_user(test_client):
    new_user = {"username": "testadmin", "password": "testpassword"}
    response = test_client.post("/api/register", json=new_user)
...
def test_login_user(test_client):
    user_credentials = {"username": "testadmin", "password": "testpassword"}
