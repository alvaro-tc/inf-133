...
from views.candy_view import render_candies_list, render_candy_detail
...
def get_candys():
    ...
    return jsonify(render_candies_list(candys))



@candy_bp.route("/candy/<int:id>", methods=["GET"])
@jwt_required
@roles_required(roles=["admin", "user"])
def get_candy(id):
...
@candy_bp.route("/candy/<int:id>", methods=["PUT"])
@jwt_required
@roles_required(roles=["admin"])
def update_candy(id):
    candy = Candy.get_by_id(id)
...
@candy_bp.route("/candy/<int:id>", methods=["DELETE"])
@jwt_required
@roles_required(roles=["admin"])
def delete_candy(id):
    candy = Candy.get_by_id(id)
